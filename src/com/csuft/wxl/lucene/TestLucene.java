...
public class TestLucene extends HttpServlet {

	public StringBuffer getSearch()  throws Exception{
		// 1. 准备中文分词器
				...
				str.append(String.format("当前一共有%d条数据%n", productNames.size()));
				System.out.printf("查询关键字是：\"%s\"%n", keyword);
				str.append(String.format("查询关键字是：\"%s\"%n", keyword));
				ScoreDoc[] hits = searcher.search(query, numberPerPage).scoreDocs;

				// 5. 显示查询结果
				str.append(showSearchResults(searcher, hits, query, analyzer));
	...
	private static StringBuffer showSearchResults(IndexSearcher searcher, ScoreDoc[] hits, Query query,
			IKAnalyzer analyzer) throws Exception {
		StringBuffer str = new StringBuffer();
		System.out.println("找到 " + hits.length + " 个命中.");
		str.append("找到 " + hits.length + " 个命中.\n");
		System.out.println("序号\t匹配度得分\t结果");
		str.append("序号\t匹配度得分\t结果\n");
